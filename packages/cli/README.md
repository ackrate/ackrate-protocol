...
The cold, fully terminal-driven flow remains testnet-only because it generates
throwaway burner keys. Mainnet uses the hosted Freighter flow so a recovery
phrase never enters the CLI.

Package version: `reapp-protocol-cli@0.1.8`. Installed command: `reapp`.
...
For the bounded mainnet USDC canary:

```bash
npx reapp-protocol-cli demo research-agent --network mainnet
```

The command prints the verified `reapp.live/wallet` entrypoint. Connect
Freighter on Stellar Mainnet, approve a 0.03 USDC mandate, and run 0.01 USDC
purchases. The hosted reference consumer uses `agent.fetch()` against the
fulfillment agent; MandateRegistry permits three purchases and rejects the
fourth. Freighter signs user authorization, and the CLI never handles the
...
| `reapp demo research-agent --network mainnet` | Opens the verified Freighter + USDC reference flow. |
