...
| `a4-ap2-adversarial.mjs` | tampered signature, mutated payload, forged issuer, wrong session user, replay, unsupported semantics, version relabelling — run against **both** the v0.1 IntentMandate and the v0.2 Open Payment Mandate profile | 41 checks: 2 valid admissions, 2 controls proving the forged signatures are genuinely valid, 26 validator rejections asserting their `Ap2ValidationError` code (exact for 20; the 6 post-signing payload mutations accept the small set of codes that mutation can legitimately produce), and 11 signing-time refusals each asserting its exact message |
