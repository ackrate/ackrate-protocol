# Ackrate hackathon quickstart — Stellar testnet
...
Use the hosted [`ackrate.live/express`](https://ackrate.live/express) workbench to
...
- Use `createBoundAckratePaidJsonRoute`; its callback receives no Express response.
...
- For external side effects, use a transactional job/outbox. Only after proving
  ...
  `resolveBoundAckrateInterruptedDelivery` API to store one terminal result; never
